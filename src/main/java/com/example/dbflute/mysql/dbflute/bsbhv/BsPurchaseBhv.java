 ...
 * 
 * [foreign table]
 *     member, product, PURCHASE, MEMBER_ADDRESS(AsSkipRelation), purchase(AsOne)
 * 
 * [referrer table]
 *     purchase
 * 
 * [foreign property]
 *     member, product, purchaseSelf, memberAddressAsSkipRelation, purchaseSelfAsOne
 * 
 * [referrer property]
 *     
 * </pre>
...
public abstract class BsPurchaseBhv extends AbstractBehaviorWritable {
    ...
    public List<Purchase> pulloutPurchaseSelf(List<Purchase> purchaseList) {
        return helpPulloutInternally(purchaseList, new InternalPulloutCallback<Purchase, Purchase>() {
            public Purchase getFr(Purchase e) { return e.getPurchaseSelf(); }
            public boolean hasRf() { return true; }
            public void setRfLs(Purchase e, List<Purchase> ls)
            { if (!ls.isEmpty()) { e.setPurchaseSelfAsOne(ls.get(0)); } }
        });
    }
    /**
    ...
    public List<MemberAddress> pulloutMemberAddressAsSkipRelation(List<Purchase> purchaseList) {
        return helpPulloutInternally(purchaseList, new InternalPulloutCallback<Purchase, MemberAddress>() {
            public MemberAddress getFr(Purchase e) { return e.getMemberAddressAsSkipRelation(); }
            public boolean hasRf() { return false; }
            public void setRfLs(MemberAddress e, List<Purchase> ls)
    ...
    public List<Purchase> pulloutPurchaseSelfAsOne(List<Purchase> purchaseList) {
        return helpPulloutInternally(purchaseList, new InternalPulloutCallback<Purchase, Purchase>() {
            public Purchase getFr(Purchase e) { return e.getPurchaseSelfAsOne(); }
            public boolean hasRf() { return true; }
            public void setRfLs(Purchase e, List<Purchase> ls)
            { if (!ls.isEmpty()) { e.setPurchaseSelf(ls.get(0)); } }
        });
    }
